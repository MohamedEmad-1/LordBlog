---
title: How to update dependencies of Evelx Logs Of A Random Dev
author: Sat Naing
...
## Updating Evelx Logs Of A Random Dev template

Like other open-source projects, Evelx Logs Of A Random Dev is evolving with bug fixes, feature updates, and so on. So if you’re someone who is using Evelx Logs Of A Random Dev as a template, you might also want to update the template when there’s a new release.
...
If you’re someone who only updates the bare minimum of the template, it should be okay to replace everything with the latest Evelx Logs Of A Random Dev except the above files and directories. It’s like pure Android OS and other vendor-specific OSes like OneUI. The less you modify the base, the less you have to update.
...
### Updating Evelx Logs Of A Random Dev using Git
...
First, add astro-ink as the remote in your project.

```bash
git remote add astro-ink https://github.com/MohamedEmad-1.git
```
...
```bash
git checkout -b build/update-astro-ink
```

Then, pull the changes from astro-ink by running

```bash
git pull astro-ink main
```

If you face `fatal: refusing to merge unrelated histories` error, you can resolve that by running the following command

```bash
git pull astro-ink main --allow-unrelated-histories
```
...
In this article, I've shared some of my insights and processes for updating dependencies and the Evelx Logs Of A Random Dev template. I genuinely hope this article proves valuable and assists you in managing your projects more efficiently.

If you have any alternative or improved approaches for updating dependencies/Evelx Logs Of A Random Dev, I would love to hear from you. Thus, don't hesitate to start a discussion in the repository, email me, or open an issue. Your input and ideas are highly appreciated!
